...
export default function AndroidTvLiveView({
  ...
  const [interval, setIntervalPreset] = useState(defaultInterval)
  ...
  const sendOverlayKey = async (keyId) => {
    const key = OVERLAY_KEYS[keyId]
    if (!key || !remoteConnected) return
    let ok = false
    if (onRemoteKey) {
      ok = await onRemoteKey(key[0], key[1])
    } else {
      try {
...
function OverlayButton({ label, ...props }) {
  return (
    <button
      type="button"
